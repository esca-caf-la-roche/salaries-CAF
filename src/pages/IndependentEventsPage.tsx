...
export function IndependentEventsPage() {
  ...
  const selectedEvents = useMemo(() => employeeEvents.filter((event) => (!event.invoiceId || event.invoiceId === editingInvoice?.id) && selectedEventIds.has(event.id)), [employeeEvents, selectedEventIds, editingInvoice])
    ...
    <div className="page unassigned-page independent-page">
      ...
      {!loading && employeeId && <section className="independent-invoices" aria-labelledby="invoices-title">
        ...
        {invoices.length === 0 ? <p>Aucune facture enregistrée pour cet indépendant.</p> : <div className="independent-invoices__list">{invoices.map((invoice) => <article key={invoice.id}>
          ...
          <div><button type="button" onClick={() => { setEditingInvoice(invoice); setSelectedEventIds(new Set(events.filter((event) => event.invoiceId === invoice.id).map((event) => event.id))) }}><Pencil aria-hidden="true" /> Modifier</button><button type="button" onClick={() => { if (window.confirm('Supprimer cette facture ? Les événements seront de nouveau disponibles.')) void (async () => { await deleteIndependentInvoice(invoice.id); setInvoices((current) => current.filter((item) => item.id !== invoice.id)); setEvents((current) => current.map((event) => event.invoiceId === invoice.id ? { ...event, invoiceId: null } : event)) })() }}><Trash2 aria-hidden="true" /> Supprimer</button></div>
        </article>)}</div>}
      </section>}
      {editingInvoice && <section className="independent-invoice" aria-label="Modifier la facture"><div><h2>Modifier la facture</h2><p>Sélectionnez ou retirez les événements directement dans la liste ci-dessous.</p></div><div className="independent-invoice__fields"><label><span>Référence de facture</span><input value={editingInvoice.invoiceNumber ?? ''} onChange={(event) => setEditingInvoice({ ...editingInvoice, invoiceNumber: event.target.value || null })} /></label><label><span>Date de réception</span><input type="date" value={editingInvoice.receivedOn} onChange={(event) => setEditingInvoice({ ...editingInvoice, receivedOn: event.target.value })} /></label><div className="independent-invoice__total"><strong>{selectedEvents.length} événement{selectedEvents.length > 1 ? 's' : ''} · {totalHours(selectedEvents)} h</strong><div className="independent-invoice__actions"><button className="button" type="button" disabled={!selectedEvents.length} onClick={() => void (async () => { await updateIndependentInvoice({ invoiceId: editingInvoice.id, invoiceNumber: editingInvoice.invoiceNumber ?? '', receivedOn: editingInvoice.receivedOn, eventIds: selectedEvents.map((event) => event.id), schoolYear }); setInvoices(await getIndependentInvoices(employeeId)); setEvents((current) => current.map((event) => event.invoiceId === editingInvoice.id || selectedEventIds.has(event.id) ? { ...event, invoiceId: selectedEventIds.has(event.id) ? editingInvoice.id : null } : event)); setEditingInvoice(null); setSelectedEventIds(new Set()) })()}>Enregistrer</button><button className="button button--secondary" type="button" onClick={() => setEditingInvoice(null)}>Annuler</button></div></div></div></section>}
                  ...
                  {dayEvents.map((event) => (
                    <section className="day-event" key={`${event.employeeId}-${event.id}`}>
                      {employeeId && <label className="independent-event-select">
                        <input type="checkbox" checked={selectedEventIds.has(event.id)} disabled={Boolean(event.invoiceId && event.invoiceId !== editingInvoice?.id)} onChange={() => toggleEvent(event.id)} aria-label={`Ajouter ${event.title} à la facture`} />
                        <span>{event.invoiceId && event.invoiceId !== editingInvoice?.id ? 'Déjà facturé' : 'Facturer'}</span>
                      </label>}
