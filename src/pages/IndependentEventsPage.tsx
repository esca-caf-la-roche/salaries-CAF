...
export function IndependentEventsPage() {
  ...
  return (
    <div className="page unassigned-page independent-page">
      ...
      {editingInvoice && <section className="independent-invoice" aria-label="Modifier la facture"><div><h2>Modifier la facture</h2><p>Sélectionnez ou retirez les événements directement dans la liste ci-dessous.</p></div><div className="independent-invoice__fields"><label><span>Référence de facture</span><input value={editingInvoice.invoiceNumber ?? ''} onChange={(event) => setEditingInvoice({ ...editingInvoice, invoiceNumber: event.target.value || null })} /></label><label><span>Date de réception</span><input type="date" value={editingInvoice.receivedOn} onChange={(event) => setEditingInvoice({ ...editingInvoice, receivedOn: event.target.value })} /></label><div className="independent-invoice__total"><strong>{selectedEvents.length} événement{selectedEvents.length > 1 ? 's' : ''} · {totalHours(selectedEvents)} h</strong><button className="button" type="button" disabled={!selectedEvents.length} onClick={() => void (async () => { await updateIndependentInvoice({ invoiceId: editingInvoice.id, invoiceNumber: editingInvoice.invoiceNumber ?? '', receivedOn: editingInvoice.receivedOn, eventIds: selectedEvents.map((event) => event.id), schoolYear }); setInvoices(await getIndependentInvoices(employeeId)); setEvents((current) => current.map((event) => event.invoiceId === editingInvoice.id || selectedEventIds.has(event.id) ? { ...event, invoiceId: selectedEventIds.has(event.id) ? editingInvoice.id : null } : event)); setEditingInvoice(null); setSelectedEventIds(new Set()) })()}>Enregistrer</button><button type="button" onClick={() => setEditingInvoice(null)}>Annuler</button></div></div></section>}
